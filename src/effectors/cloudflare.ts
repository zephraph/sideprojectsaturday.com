...
import { R2Object, R2ObjectBody, R2Bucket, R2Checksums, R2HTTPMetadata, R2Range, R2GetOptions, R2Conditional, R2PutOptions, R2MultipartOptions, R2Objects, R2MultipartUpload, R2ListOptions } from "@cloudflare/workers-types";
...
type OperationR2Bucket = {
  [K in keyof R2Bucket]: R2Bucket[K] extends (...args: any[]) => Promise<infer R>
  ? (...args: Parameters<R2Bucket[K]>) => Operation<R extends R2ObjectBody ? OperationR2ObjectBody : R>
  : R2Bucket[K]
};
...
export class R2BucketEffects implements OperationR2Bucket {
  ...
  get(key: string, options?: R2GetOptions): Operation<OperationR2ObjectBody | null>;
  get(key: string, options: R2GetOptions & { onlyIf: R2Conditional | Headers }): Operation<OperationR2ObjectBody | R2Object | null>;
  get(key: string, options?: R2GetOptions & { onlyIf?: R2Conditional | Headers }): Operation<OperationR2ObjectBody | R2Object | null> {
    return call(async () => {
      const result = await this.bucket.get(key, options);
      return result && 'body' in result ? new OperationR2ObjectBody(result) : result;
    });
  }

  put(key: string, value: ReadableStream | ArrayBuffer | ArrayBufferView | string | null | Blob, options?: R2PutOptions): Operation<R2Object>;
  put(
    key: string,
  ...
  createMultipartUpload(key: string, options?: R2MultipartOptions): Operation<R2MultipartUpload> {
